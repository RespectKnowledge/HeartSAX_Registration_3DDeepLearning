# -*- coding: utf-8 -*-
"""
Created on Tue Jul 11 11:10:04 2023
...
dataset_train=regsax(path,pathlab,(128,128,32))
...
pathlab='/home/aqayyum/SAXregistration/labels'
dataset_valid=regsax(path,pathlab,(128,128,32))
#img1,img2,lab1m,lab2m=datatrain[0]
...
model_idx = -1
weights = [1, 1]
#model_folder = 'TransMorph_ncc_{}_dsc1_diffusion_{}/'.format(weights[0], weights[1])
#model_dir = 'experiments/' + model_folder
model_dir='/home/aqayyum/SAXregistration/experiments/TransMorph_ncc_1_dsc1_diffusion_1/'
...
test_loader = DataLoader(dataset_valid, batch_size=1, shuffle=False, num_workers=1, pin_memory=True, drop_last=True)
eval_dsc_def = AverageMeter()
eval_dsc_raw = AverageMeter()
eval_det = AverageMeter()


save_pathm='/home/aqayyum/SAXregistration/output_model1/moved'
save_pathf='/home/aqayyum/SAXregistration/output_model1/flowf'
save_pathjf='/home/aqayyum/SAXregistration/output_model1/jfield'
save_pathseg='/home/aqayyum/SAXregistration/output_model1/segout'
fixedimpath='/home/aqayyum/SAXregistration/output_model1/fixed'
movedimpath='/home/aqayyum/SAXregistration/output_model1/moving'
fixedlabelpa='/home/aqayyum/SAXregistration/output_model1/fixed_label'
movedlabelpa='/home/aqayyum/SAXregistration/output_model1/move_label'
import pandas as pd
import os
import numpy as np
import nibabel as nib
path=pd.read_csv('/home/aqayyum/SAXregistration/kfold/val_0.csv')
print(path.columns)
pathlabel='/home/aqayyum/SAXregistration/labels'
for i in range(0,len(path['fixed'])):
  pathfixed=path['fixed'][i]
  print(pathfixed)
  fixedpat=pathfixed.split('/')[-1].split('.')[0]
  pathes=pathfixed.replace('ED','ES')
  pat=pathes.split('/')[-1].split('.')[0]
  fixed_label=os.path.join(pathlabel,pathfixed.split('/')[-1].replace('ED','ED_label_gt'))
  move_label=os.path.join(pathlabel,pathes.split('/')[-1].replace('ES','ES_label_gt'))
  #print(fixed_label)
  #fimg=nib.load(fixed_label).get_fdata()
  #print(np.unique(fimg))
  #mimg=nib.load(move_label).get_fdata()
  #print(np.unique(mimg))
  #pathfixed=self.df['fixed'][index]
  #pathmove=pathfixed.replace('ED','ES')

  fixed_img=sitk.ReadImage(pathfixed)
  move_img=sitk.ReadImage(pathes)
  fixed_array=sitk.GetArrayFromImage(fixed_img).swapaxes(2, 0)
  moved_array=sitk.GetArrayFromImage(move_img).swapaxes(2, 0)
    
  fixed_array_resize=resize(fixed_array, output_shape=(128,128,32),preserve_range=True, anti_aliasing=False,order=3)
  move_array_resize=resize(moved_array, output_shape=(128,128,32),preserve_range=True, anti_aliasing=False,order=3)
  fixedimg=torch.from_numpy(fixed_array_resize).unsqueeze(0).unsqueeze(0)
  moveimg=torch.from_numpy(move_array_resize).unsqueeze(0).unsqueeze(0)
  np.save(os.path.join(fixedimpath,fixedpat+'.npy'),fixed_array_resize)
  np.save(os.path.join(movedimpath,pat+'.npy'),move_array_resize)

  ################## read labels ###################
  fixed_label=os.path.join(fixed_label)
  move_label=os.path.join(move_label)
  ...
  fixed_array_resize_la=resize(fixed_arrayla, output_shape=(128,128,32),preserve_range=True, anti_aliasing=False,order=0)
  move_array_resize_la=resize(moved_arrayla, output_shape=(128,128,32),preserve_range=True, anti_aliasing=False,order=0)
  
  np.save(os.path.join(fixedlabelpa,fixedpat+'.npy'),fixed_array_resize_la)
  np.save(os.path.join(movedlabelpa,pat+'.npy'),move_array_resize_la)
  ...
  with torch.no_grad():
    ...
    x = fixedimg.cuda()
    y = moveimg.cuda()
    x_seg = fixedlabel.cuda()
    y_seg = movelabel.cuda()
    ...
    x_def1=x_def.squeeze(0).squeeze(0)
    moved=x_def1.detach().cpu().numpy()
    print(moved.shape)
    np.save(os.path.join(save_pathm,pat+'.npy'),moved)
    #print(flow.shape)
    dflow=flow.squeeze(0)
    ddflow=dflow.detach().cpu().numpy()
    print(ddflow.shape)
    np.save(os.path.join(save_pathf,pat+'.npy'),ddflow)
    x_segs = []
    for i in range(4):
    ...
    #print(def_out.shape)
    def_out1=def_out.squeeze(0).squeeze(0).detach().cpu().numpy()
    print(def_out1.shape)
    np.save(os.path.join(save_pathseg,pat+'.npy'),def_out1)
    del x_segs, x_seg_oh
    #def_out = reg_model([x_seg.cuda().float(), flow.cuda()])
    tar = y.detach().cpu().numpy()[0, 0, :, :, :]
    jac_det = jacobian_determinant_vxm(flow.detach().cpu().numpy()[0, :, :, :, :])
    print(jac_det.shape)
    np.save(os.path.join(save_pathjf,pat+'.npy'),jac_det)
    ...
    dsc_trans = dice_val(def_out.long(), y_seg.long(), 4)
    dsc_raw = dice_val(x_seg.long(), y_seg.long(), 4)
...
#         model.eval()
#         pat=test_loader.dataset.df['fixed'].split('/')[-1].split('.')[0]
